...
def cargar_cookies(path=None):
    if path is None:
        path = os.path.abspath(os.path.join(os.path.dirname(
            __file__), '..', '..', 'assets', 'cookies_fdp.json'))
...
def buscar_fdp_httpx(keywords, exacta=False):
    query = keywords.replace(" ", "%20")
    url = f"https://www.farmaciasdelpueblo.com.ar/api/catalog_system/pub/products/search/?ft={query}"
    ...
    headers = {
        ...
        "Referer": "https://www.farmaciasdelpueblo.com.ar/",
        "Origin": "https://www.farmaciasdelpueblo.com.ar"
    }
    ...
    try:
        with httpx.Client(http2=True, headers=headers, cookies=cookies, timeout=10.0) as client:
            ...
            print(f"🛰️ [FDP] Status: {response.status_code}")
            ...
            for producto in productos:
                for item in producto.get("items", []):
                    ...
                    link = f"https://www.farmaciasdelpueblo.com.ar/{producto.get('linkText')}/p"
...
if __name__ == "__main__":
    resultados = buscar_fdp_httpx("tio nacho", exacta=False)
    if not resultados:
